...
export default function ExpensesPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Expenses"
        description="Track cleaning-related expenses"
        actions={
          <Button size="sm" onClick={() => setShowForm(!showForm)}>
            {showForm ? <><X className="h-4 w-4 mr-1" /> Cancel</> : <><Plus className="h-4 w-4 mr-1" /> Add Expense</>}
          </Button>
        }
      />
          ...
          <Card key={exp.id}>
            <CardContent className="flex items-center justify-between p-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground">{format(new Date(exp.date), "MMM d, yyyy")} · {exp.shop || "—"}</p>
              </div>
